...
這是一個手機優先的靜態 Web App，會從 Google Sheet 透過 Google Apps Script 取得 JSON，並在前端統計「收入」「開銷」「貸款」「資產」「季投資盈虧」與「月報」「季報」「季現金流」「年報」「年現金流」報表分頁。
...
- 從 `季_投資盈虧表` 第 1 列讀取季度，每季使用六欄：上季結算、本季投入、本季贖回、本季結算、本季損益、報酬率。
- 第 3 列到「投資合計」前讀取各投資項目，保留投資分類、項目與備註。
- 前端顯示季度總損益、報酬率、期末投資資產、資金變化、損益貢獻與各項投資卡片。
- 日期仍含 `??` 的未來季度不會顯示；檢核不為零時會顯示警告。
- 當項目的期初結算與本季投入皆為零、但已有期末金額或損益時，報酬率顯示為「無比較基準」。
